//! Core lease state machine and SQLite operations for Litelease.
//!
...
//! already owns the surrounding transaction or savepoint and wants Litelease
...
pub enum Error {
    ...
    #[error("litelease_resources schema mismatch: {reason}")]
...
/// Canonical CREATE TABLE statement for `litelease_resources`. Used both to
/// install a fresh schema and as the reference shape for validating an
/// existing one.
const LITELEASE_RESOURCES_DDL: &str = "CREATE TABLE litelease_resources (
...
/// Install Litelease's schema on `conn`, or validate it strictly if a
/// `litelease_resources` table already exists.
///
/// On a fresh database the canonical schema is created. On a database that
/// already has a `litelease_resources` table, the existing table is validated
...
pub fn bootstrap_litelease_schema(conn: &Connection) -> Result<()> {
    if litelease_resources_table_exists(conn)? {
        validate_existing_schema(conn)?;
    } else {
        conn.execute_batch(LITELEASE_RESOURCES_DDL)?;
    }
    Ok(())
}

fn litelease_resources_table_exists(conn: &Connection) -> Result<bool> {
    let count: i64 = conn.query_row(
        "SELECT COUNT(*) FROM sqlite_master \
         WHERE type = 'table' AND name = 'litelease_resources'",
        [],
        |row| row.get(0),
    )?;
    Ok(count > 0)
...
fn validate_columns(conn: &Connection) -> Result<()> {
    let mut stmt = conn.prepare(
        "SELECT cid, name, type, [notnull], pk \
         FROM pragma_table_info('litelease_resources') \
         ORDER BY cid",
    )?;
...
fn validate_table_checks(conn: &Connection) -> Result<()> {
    ...
    let sql = sql.ok_or_else(|| Error::SchemaMismatch {
        reason: "litelease_resources has no stored CREATE TABLE text".to_owned(),
    })?;
...
pub fn claim_in_tx(
    ...
    match load_resource(conn, name)? {
        None => {
            conn.execute(
                "INSERT INTO litelease_resources (
                   name,
                   owner,
                   token,
                   lease_expires_at_ms,
...
/// Register all `litelease_*` scalar functions on `conn`.
///
...
pub fn attach_litelease_functions(conn: &Connection) -> rusqlite::Result<()> {
    conn.create_scalar_function("litelease_bootstrap", 0, FunctionFlags::SQLITE_UTF8, |ctx| {
        let db = unsafe { ctx.get_connection() }?;
        bootstrap_litelease_schema(&db).map_err(to_sql_err)?;
        Ok(1i64)
    })?;

    conn.create_scalar_function("litelease_claim", 4, FunctionFlags::SQLITE_UTF8, |ctx| {
    ...
    conn.create_scalar_function("litelease_renew", 4, FunctionFlags::SQLITE_UTF8, |ctx| {
        let name: String = ctx.get(0)?;
        let owner: String = ctx.get(1)?;
        let ttl_ms: i64 = ctx.get(2)?;
        let now_ms: i64 = ctx.get(3)?;
    ...
    conn.create_scalar_function("litelease_release", 3, FunctionFlags::SQLITE_UTF8, |ctx| {
        let name: String = ctx.get(0)?;
        let owner: String = ctx.get(1)?;
    ...
    conn.create_scalar_function("litelease_owner", 2, FunctionFlags::SQLITE_UTF8, |ctx| {
        let name: String = ctx.get(0)?;
    ...
    conn.create_scalar_function("litelease_token", 1, FunctionFlags::SQLITE_UTF8, |ctx| {
        let name: String = ctx.get(0)?;
        let db = unsafe { ctx.get_connection() }?;
...
mod tests {
    ...
    fn open_db() -> Connection {
        ...
        bootstrap_litelease_schema(&conn).expect("bootstrap schema");
        conn
    }
    ...
    fn open_shared_db() -> (TempDir, Connection, Connection) {
        ...
        bootstrap_litelease_schema(&conn_a).expect("bootstrap schema on first connection");

        let conn_b = Connection::open(&db_path).expect("open second sqlite connection");
        bootstrap_litelease_schema(&conn_b).expect("bootstrap schema on second connection");
    ...
    fn open_shared_sql_db() -> (TempDir, Connection, Connection) {
        ...
        attach_litelease_functions(&conn_a).expect("attach functions to first sql connection");
        bootstrap_litelease_schema(&conn_a).expect("bootstrap schema on first sql connection");
        ...
        attach_litelease_functions(&conn_b).expect("attach functions to second sql connection");
        bootstrap_litelease_schema(&conn_b).expect("bootstrap schema on second sql connection");
    ...
    fn bootstrap_is_idempotent() {
        ...
        bootstrap_litelease_schema(&conn).unwrap();
        bootstrap_litelease_schema(&conn).unwrap();
    ...
    fn attached_sql_functions_cover_bootstrap_and_full_lease_cycle() {
        ...
        let token_after_claim: Option<i64> = conn
            .query_row("SELECT litelease_token(?1)", params!["scheduler"], |row| {
                row.get(0)
            })
            .unwrap();
