...
use litelease_core::{
    attach_litelease_functions, bootstrap_litelease_schema, claim, claim_in_tx, inspect, release_in_tx,
...
fn open_core_conn(path: &Path, journal_mode: JournalMode, busy_timeout: Duration) -> Connection {
    ...
    bootstrap_litelease_schema(&conn).expect("bootstrap litelease schema");
    conn
}
...
fn assert_core_claim(result: litelease_core::Result<ClaimResult>, expect: Expect) {
    match (result, expect) {
...
fn assert_core_lock_failure<T: std::fmt::Debug>(result: litelease_core::Result<T>) {
    match result {
...
fn core_error_is_busy_or_locked(err: &litelease_core::Error) -> bool {
    match err {
        litelease_core::Error::Sqlite(err) => sqlite_error_is_busy_or_locked(err),
        _ => false,
    }
}
...
#[test]
fn core_begin_immediate_takes_writer_intent_before_litelease_mutation() {
    let db = DbFile::fresh();
    let conn_a = open_core_conn(&db.path, JournalMode::Delete, Duration::from_millis(0));
    let conn_b = open_core_conn(&db.path, JournalMode::Delete, Duration::from_millis(0));
    ...
    let err = conn_b
        .execute_batch("BEGIN IMMEDIATE")
        .expect_err("conn_b should fail before litelease mutation");
...
#[test]
fn sql_begin_immediate_takes_writer_intent_before_litelease_mutation() {
    let db = DbFile::fresh();
    let conn_a = open_sql_conn(&db.path, JournalMode::Delete, Duration::from_millis(0));
    let conn_b = open_sql_conn(&db.path, JournalMode::Delete, Duration::from_millis(0));
