//! Phase 011 deterministic invariant runner for `litelease-core`.
//!
...
//! - public `litelease-core` API for lease behavior; direct table reads only
...
fn read_row(conn: &Connection, name: &str) -> Option<(Option<String>, i64, Option<i64>)> {
    conn.query_row(
        "SELECT owner, token, lease_expires_at_ms FROM litelease_resources WHERE name = ?1",
        params![name],
        |row| {
    ...
    .optional()
    .expect("read litelease_resources row")
}
