...
//! between rows. The matrix proves Litelease does not set, rewrite, or
...
use litelease_core::{
    attach_litelease_functions, bootstrap_litelease_schema, claim, claim_in_tx, ClaimResult,
};
...
fn assert_same_conn_unchanged(before: &PragmaState, conn: &Connection) {
    ...
    assert_eq!(
        before, &after,
        "same-connection pragmas changed after Litelease operation"
    );
}
...
fn sql_bootstrap_leaves_pragmas_alone() {
    ...
    let before = set_pragmas(&conn);
    attach_litelease_functions(&conn).expect("attach functions");

    let bootstrapped: i64 = conn
        .query_row("SELECT litelease_bootstrap()", [], |row| row.get(0))
        .expect("litelease_bootstrap");
...
fn sql_savepoint_claim_leaves_pragmas_alone() {
    ...
    let token: Option<i64> = conn
        ...
        .expect("litelease_claim in savepoint");
    assert_eq!(token, Some(1));
