...
use litelease_core::{
    bootstrap_litelease_schema, claim, inspect, owner as core_owner, release, renew, token,
...
fn assert_schema_mismatch(result: litelease_core::Result<()>) -> String {
    match result {
...
fn bootstrap_creates_fresh_schema_when_table_absent() {
    ...
    bootstrap_litelease_schema(&conn).expect("bootstrap fresh DB");
...
fn bootstrap_is_idempotent_on_valid_schema() {
    ...
    bootstrap_litelease_schema(&conn).expect("second bootstrap");
    bootstrap_litelease_schema(&conn).expect("third bootstrap");
}
...
fn bootstrap_preserves_existing_live_lease() {
    ...
    bootstrap_litelease_schema(&conn).expect("re-bootstrap on valid schema");
...
fn bootstrap_inside_caller_owned_transaction_is_valid() {
    ...
    bootstrap_litelease_schema(&conn).expect("bootstrap inside tx is read-only");
    ...
    bootstrap_litelease_schema(&conn).expect("post-rollback bootstrap still valid");
}
...
fn bootstrap_rejects_table_with_extra_column() {
    ...
    let reason = assert_schema_mismatch(bootstrap_litelease_schema(&conn));
    assert!(
...
/// Create a `litelease_resources` table without the row-level CHECK
/// constraints so tests can persist rows that Litelease must reject. This
/// fixture deliberately produces a schema Litelease's bootstrap would reject;
...
fn assert_invalid_lease_row(result: litelease_core::Result<impl std::fmt::Debug>) {
    match result {
...
fn read_raw_token(conn: &Connection, name: &str) -> i64 {
    conn.query_row(
        "SELECT token FROM litelease_resources WHERE name = ?1",
        params![name],
        |row| row.get(0),
    )
    .expect("read token")
...
fn round_trip_punctuation_and_sql_shaped_strings() {
    assert_round_trip(
        "a/b/c, scheduler #1",
        "'; DROP TABLE litelease_resources; --",
    );
}
