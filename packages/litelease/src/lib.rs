...
//! machine.
//!
//! Use [`Litelease`] when you want the wrapper to own the connection.
//! Use [`LiteleaseRef`] when you already own the `rusqlite::Connection`
//! or current transaction boundary and want Litelease to participate on
...
#[derive(Debug)]
pub struct Litelease {
    conn: Connection,
}

#[derive(Debug, Clone, Copy)]
pub struct LiteleaseRef<'a> {
    conn: &'a Connection,
}
...
impl Litelease {
    ...
    pub fn bootstrap(&self) -> Result<()> {
        core::bootstrap_litelease_schema(&self.conn)?;
        Ok(())
    }

    pub fn borrowed(&self) -> LiteleaseRef<'_> {
        LiteleaseRef::new(&self.conn)
    }

    /// Begin a `BEGIN IMMEDIATE` transaction for atomic business writes
    /// plus Litelease lease mutations on this connection.
