...
#[test]
fn loadable_extension_registers_all_litelease_functions() -> rusqlite::Result<()> {
...
fn assert_full_smoke_path(conn: &Connection) -> rusqlite::Result<()> {
    let bootstrapped: i64 = conn.query_row("SELECT litelease_bootstrap()", [], |row| row.get(0))?;
    ...
    let token: Option<i64> =
        conn.query_row("SELECT litelease_token(?1)", ("scheduler",), |row| row.get(0))?;
...
fn build_extension_artifact(profile: BuildProfile) -> PathBuf {
    ...
    let status = command
        ...
        .status()
        .expect("run cargo build for litelease-extension");
    assert!(
        status.success(),
        "cargo build -p litelease-extension{} failed",
        if matches!(profile, BuildProfile::Release) {
            " --release"
        } else {
            ""
    ...
    let artifact = target_dir(&repo_root)
        .join(profile.as_str())
        .join(format!("{DLL_PREFIX}litelease_ext{DLL_SUFFIX}"));
    assert!(
        artifact.exists(),
        "missing litelease extension artifact at {}",
        artifact.display()
    );
    artifact
}
...
fn repo_root() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .expect("packages/litelease lives under repo root")
        .to_path_buf()
}
