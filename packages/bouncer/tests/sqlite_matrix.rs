...
fn open_conn(path: &Path, journal_mode: JournalMode, busy_timeout: Duration) -> Connection {
    ...
    core::bootstrap_bouncer_schema(&conn).expect("bootstrap bouncer schema");
    conn
}
...
fn wrapper_transaction_claims_writer_intent_up_front() {
    let db = DbFile::fresh();
    let mut wrapper_a = Bouncer::open(&db.path).expect("open wrapper_a");
    ...
    let err = contender
        ...
        .expect_err("contending writer should fail before bouncer mutation");
    assert!(
...
#[test]
fn bouncer_ref_deferred_begin_mirrors_core_lock_upgrade_behavior() {
    let db = DbFile::fresh();
    ...
    let borrowed_a = BouncerRef::new(&conn_a);
    let borrowed_b = BouncerRef::new(&conn_b);
...
#[test]
fn bouncer_ref_autocommit_live_lease_returns_lease_busy_in_wal_journal() {
    let db = DbFile::fresh();
...
#[test]
fn bouncer_ref_savepoint_participates_in_caller_owned_boundary() {
    let db = DbFile::fresh();
