...
//!
//! Proves Bouncer behaves the way a normal user would expect
...
fn user_journey_002_second_caller_sees_busy() {
    let db = DbFile::fresh();
    let wrapper_a = Bouncer::open(&db.path).expect("open wrapper_a");
    ...
    let wrapper_b = Bouncer::open(&db.path).expect("open wrapper_b");
...
fn user_journey_005_cross_surface_interop() {
    ...
    attach_bouncer_functions(&sql_conn).expect("attach sql functions");
    bootstrap_bouncer_schema(&sql_conn).expect("bootstrap via sql");
...
fn user_journey_006_caller_owned_transaction_atomic_visibility() {
    ...
    let lease: Option<String> = observer
        .query_row(
            "SELECT owner FROM bouncer_resources WHERE name = ?1",
            params![NAME],
            |row| row.get::<_, Option<String>>(0),
        )
        .optional()
        .expect("read bouncer_resources")
        .flatten();
    assert!(
        lease.is_none(),
    ...
    let observer_borrowed = BouncerRef::new(&observer2);
...
fn user_journey_007_drifted_schema_fails_loudly() {
    ...
    let sql_conn = fresh_conn(&db.path);
    attach_bouncer_functions(&sql_conn).expect("attach functions");
